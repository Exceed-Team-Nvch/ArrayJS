...
arr.map((item, index, array) => { if (item.length == 4 ) console.log(`Метод map , элемент у которого длина равна 4 - ${item} и его индекс - ${index}`)});
 
///метод forEach просто перебирает массив
arr.forEach((item,index,array) => console.log(`Метод forEach : Элемент: ${item} и его индекс: ${index}`));

let filterResult = arr.filter((item,index,array) => {if (index% 2 == 0)  return item});

console.log(`Метод filter: все четные и нулевой элементы массива : ${filterResult.toString()}`);

let reduceResult = arr.reduce((prev,item,index) => prev + item );

console.log(`Метод reduce: одна большая строчка из элементов массива: ${reduceResult}`);

console.log(`Метод sort:  ${arr.sort()}`);

let isHtml = item  =>  item == `HTML`;
